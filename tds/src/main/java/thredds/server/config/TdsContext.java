...
public final class TdsContext implements ServletContextAware, InitializingBean, DisposableBean {
  ...
  public void afterPropertiesSet() {
    ...
    if (rootPath == null) {
      String msg =
          "Webapp [" + this.webappDisplayName + "] must run with exploded deployment directory (not from .war).";
    ...
    if (!contentRootDir.isAbsolute())
      this.threddsDirectory =
          new File(new File(this.servletRootDirectory, this.contentRootPathProperty), this.contentPathProperty);
    else {
    ...
    if (this.threddsDirectory.isDirectory()) {
      ...
      String message = String.format("TdsContext.init(): Content directory is not a directory: %s",
          this.threddsDirectory.getAbsolutePath());
      logServerStartup.error(message);
      throw new IllegalStateException(message);
    }
    ...
    try {
      ...
      if (!catalogFile.exists()) {
        ...
        logServerStartup.info("TdsContext.init(): Copying default enhanced catalog file from {}.",
            defaultEnhancedCatalogFile);
      ...
      if (!threddsConfigFile.exists()) {
        ...
        logServerStartup.info("TdsContext.init(): Copying default THREDDS config file from {}.",
            defaultThreddsConfigFile);
      ...
      if (!customTemplateFragmentsFile.exists()) {
        ...
        logServerStartup.info("TdsContext.init(): Copying default template fragments file from {}.",
            defaultTemplateFragmentsFile);
